...
- Removed the standalone Vue/Vuetify package, Node toolchain, UI CI workflow, nginx image, and `ui-*` container tags.

## [2.0.0] - 2026-08-28

### Added

- Added a typed Python analysis engine under `hopla-py/` with Typer CLI subtools `run`, `convert`, `concordance`, and `transform`.
...
- Added conversion from the legacy `key=value` settings format toschema-validated YAML.
...
- Added pixi `hopla-py` / `hopla-py-dev` environments, pytest coverage, and ruff / mypy lint tasks.
...
- Settings keys `color_palette`, `self_contained`, and `cairo` remain accepted for schema compatibility; the Python report always emits a single offline HTML file with the Paired palette and does not use a cairo device.
