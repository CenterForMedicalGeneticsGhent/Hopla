...
def weighted_vote(values: np.ndarray, positions: np.ndarray, max_distance: float) -> np.ndarray:
    ...
    for index, position in enumerate(positions):
        left = int(np.searchsorted(positions, position - max_distance, side="left"))
        right = int(np.searchsorted(positions, position + max_distance, side="right"))
        distances = np.abs(positions[left:right].astype(np.float64) - position)
...
def correct_short_segments(values: np.ndarray, genotypes: np.ndarray, minimum: int) -> np.ndarray:
    ...
    for segment, (left, right) in enumerate(zip(boundaries[:-1], boundaries[1:], strict=True)):
        if np.count_nonzero(genotypes[left:right] != "NA") > minimum:
            continue
        replacement = result[left - 1] if left else (
            result[right] if right < result.size else result[left]
        )
...
def correct_haplotypes(
    ...
    for strand in range(2):
        ...
        corrected[:, strand] = weighted_vote(
            corrected[:, strand], positions, window_size / 2
        )
...
def _alleles(
    ...
    for sample_index in range(len(matrix.samples)):
        ...
        for local, site_index in enumerate(source):
            ...
            sample_rows.append(
                f"{ref}/{ref}" if gt == 0 else f"{ref}/{alt}" if gt == 1 else f"{alt}/{alt}" if gt == 2 else "N/N"
            )
...
def _write_inputs(
    ...
    for x_mode, suffix in ((False, ""), (True, "X")):
        ...
        mapping = "".join(
            f"{sites.chrom[source[index]] if not x_mode else 23}\tid{source[index] + 1}\t{sites.pos[source[index]] / 1_000_000}\n"
            for index in selected_indices
        )
        pedigree_rows = []
...
def _run_one(directory: Path, executable: str, suffix: str, model: str) -> None:
    ...
    with (directory / f"merlin{suffix}.o").open("w", encoding="utf-8") as output:
        subprocess.run([executable, *common, "--error", "--prefix", str(prefix)], check=True, stdout=output)
    errors = Path(f"{prefix}.err")
    rejected = set()
    if errors.exists() and errors.stat().st_size:
        try:
            rejected = set(pl.read_csv(errors, separator=" ", infer_schema_length=0).select_at_idx(2).to_list())
...
def _parse_blocks(path: Path, samples: tuple[str, ...]) -> dict[str, np.ndarray]:
    ...
    for block_index, block in enumerate(blocks):
        ...
        for line in lines:
            if "(" in line:
                active = [token for token in re.sub(r"\([^)]*\)", "", line).split() if token in sample_values]
                continue
...
def run_merlin(
    ...
    for chrom, flow_matrix in flow.items():
        ...
        for sample_index, sample in enumerate(matrix.samples):
            ...
            window = settings.window_size_voting_x if chrom == "chrX" else settings.window_size_voting
